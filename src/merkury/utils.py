...
def _get_default_file_name(script_file_path: Path, output_format: str, include_date: bool) -> str:
    """
    Default file name
    """
...
def _get_default_path(script_file_path: Path, output_format: str, include_date: bool) -> Path:
    """
    Default file path for report
    """
    out_file_name = _get_default_file_name(script_file_path, output_format, include_date)
...
def get_report_path(script_file_path: Path, specified_output: Path | None, output_format: str, include_date: bool) -> Path:
    """
    Get report path
    """
    if specified_output is None:
        report_file_path = _get_default_path(script_file_path, output_format, include_date)
    else:
        if specified_output.is_dir():
            file_name = _get_default_file_name(script_file_path, output_format, include_date)
