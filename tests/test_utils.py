...
def test_get_default_path():
    ...
    assert re.search(r"^rel/file_.*\.pdf$", str(get_default_path(pathlib.Path("rel/file.sql"), "pdf")))
    assert re.search(r"^rel/filesql_.*\.html$", str(get_default_path(pathlib.Path("rel/filesql"), "html")))
