...
def test_get_default_file_name():
    assert (
        str(_get_default_file_name(Path("rel/file.py"), "html", False)) == "file.html"
    )
    assert (
        str(_get_default_file_name(Path("/abs/file.py"), "html", False)) == "file.html"
    )
    assert re.search(
        r"^file_.*\.html$",
        str(_get_default_file_name(Path("rel/file.py"), "html", True)),
    )
    assert re.search(
        r"^file_.*\.html$",
        str(_get_default_file_name(Path("/abs/file.py"), "html", True)),
    )
...
def test_get_default_path():
    ...
    assert re.search(
        r"^/abs/path/file_.*\.html$",
        str(_get_default_path(Path("/abs/path/file.py"), "html", True)),
    )
    assert re.search(
        r"^rel/file_.*\.pdf$", str(_get_default_path(Path("rel/file.py"), "pdf", True))
    )
    assert re.search(
        r"^rel/filepy_.*\.html$",
        str(_get_default_path(Path("rel/filepy"), "html", True)),
    )
    assert (
        str(_get_default_path(Path("/abs/path/file.py"), "html", False))
        == "/abs/path/file.html"
    )
    assert str(_get_default_path(Path("rel/file.py"), "pdf", False)) == "rel/file.pdf"
    assert (
        str(_get_default_path(Path("rel/filepy"), "html", False)) == "rel/filepy.html"
    )
...
def test_get_report_path(tmp_path):
    ...
    script_path.touch()
    assert get_report_path(script_path, None, "html", False) == Path(
        tmp_path, "script.html"
    )
    passed_path_file = Path(tmp_path, "customdir1/customdir1/custom.html")
    assert (
        get_report_path(script_path, passed_path_file, "html", False)
        == passed_path_file
    )
